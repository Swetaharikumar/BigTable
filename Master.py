...
class MasterHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if dict_return["function_name"] == master_const.post_function_types[0]:
        	...
        	url_tablet =  MasterSupport.url(master_const.tablets_info[tablet_name]["hostname"], master_const.tablets_info[tablet_name]["port"], "/Create/")
        	hostandport = {}
        ...
        # if dict_return["function_name"] == master_const.post_function_types[1]:
        # 	print("insert")
        # 	for tablet in master_const.server_table_dict:
        # 		if dict_return["table_name"] in master_const.server_table_dict[tablet]:
        # 			tablet_name = tablet
        # 			url_tablet = MasterSupport.url(master_const.tablets_info[tablet_name]["hostname"], master_const.tablets_info[tablet_name]["port"],'/Insert/' + dict_return["table_name"] + '/')
        # 			response = requests.post(url_tablet,json=table_dict)
        # 			self._set_response(response.status_code)
        # 	self._set_response(409)
    ...
    def do_DELETE(self):
    	...
    	if dict_return["function_name"] == master_const.del_function_types[0]:
    		if dict_return["table_name"] not in master_const.table_names['tables']:
    			self._set_response(404)
    			return
    		if dict_return["table_name"] in master_const.locks and len(master_const.locks[dict_return["table_name"]]) != 0:
    			self._set_response(409)
    			return
    		...
    		for tablet in master_const.server_table_dict:
    			if dict_return["table_name"] in master_const.server_table_dict[tablet]:
    				...
    				tablet_name = tablet
    				url_tablet =  MasterSupport.url(master_const.tablets_info[tablet_name]["hostname"], master_const.tablets_info[tablet_name]["port"], "/Delete/" + str(dict_return["table_name"] +'/'))
    				print(url_tablet)
        ...
        #CLose the lock
    		...
    		if dict_return["table_name"] not in master_const.table_names["tables"]:
    			self._set_response(404)
    			return
    		if dict_return["table_name"] in master_const.locks and client_data["client_id"] not in master_const.locks[dict_return["table_name"]]:
    			self._set_response(400)
    			return
